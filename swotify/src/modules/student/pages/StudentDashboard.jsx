    ...
    <div className="flex min-h-screen bg-gradient-to-br from-cyan-700 via-teal-600 to-cyan-500 text-white animate-gradient-background">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 ${isSidebarOpen ? 'w-64' : 'w-20'} bg-teal-950 shadow-lg p-4 flex-col transition-all duration-300 ease-in-out overflow-hidden flex`}>
        <div className="flex items-center justify-center h-16 mb-8">
          {isSidebarOpen ? (
            <div className="text-2xl font-bold">Swotify</div>
          ) : (
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? 'lg:ml-64' : 'lg:ml-20'}`}>
        ...
        <header className="w-full p-4 bg-teal-700/50 backdrop-blur-sm shadow-md flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4"> {/* Container for notification and profile */}
            ...
            <div className="relative">
              ...
              {isNotificationOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-teal-900 rounded-md shadow-lg py-1 z-20 text-gray-100">
                  <div className="px-4 py-2 text-sm">
                    <p className="font-semibold">Important Update!</p>
                    <p className="text-gray-100">Your Math assignment is due tomorrow.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-teal-900 rounded-md shadow-lg py-1 z-20">
                <NavLink
                  to="profile"
                  onClick={() => setIsDropdownOpen(false)}
                  className="block px-4 py-2 text-sm text-gray-100 hover:bg-white/10"
                >
                  Manage Profile
                </NavLink>
                <NavLink
        ...
        <main className="flex-1 p-6">
          <div className="bg-teal-900 rounded-2xl shadow-2xl p-8 lg:p-10 text-gray-100 h-full">
