...
const TeacherSidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  const getColorClasses = (isActive) => {
    if (isActive) {
      return 'bg-[#ea580c] text-white shadow-xl shadow-orange-500/20';
    }
    return 'text-slate-600 hover:bg-orange-50 hover:text-[#ea580c]';
  };
  ...
  return (
    <>
      ...
      >
        {/* Logo Section */}
        <div className={`${isSidebarOpen ? 'h-20' : 'h-20'} flex items-center ${isSidebarOpen ? 'px-6' : 'justify-center'} border-b-2 border-slate-100 bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 flex-shrink-0 relative overflow-hidden`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                to={item.to}
                className={
                  `group relative flex items-center ${isSidebarOpen ? 'gap-3 px-4 py-3.5' : 'justify-center py-3'} rounded-xl transition-all duration-300 ${getColorClasses(active)} ${active ? 'scale-105' : 'hover:scale-105'}`
                }
                onClick={() => {
              ...
              >
                ...
                {isSidebarOpen && item.badge && (
                  <span className={`px-2 py-1 text-xs font-bold rounded-lg ${active
        ...
        <div className={`${isSidebarOpen ? 'p-4' : 'px-2 py-3'} border-t-2 border-slate-100 bg-white flex-shrink-0 space-y-2`}>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group relative flex items-center ${isSidebarOpen ? 'gap-3 px-4 py-3.5' : 'justify-center py-3'} rounded-xl transition-all duration-300 ${getColorClasses(isActive)} ${isActive ? 'scale-105' : 'hover:scale-105'}`
            }
            onClick={() => {
              if (window.innerWidth < 1024) {
                toggleSidebar(false);
